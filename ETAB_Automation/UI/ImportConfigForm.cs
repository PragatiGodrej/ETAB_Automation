...
namespace ETAB_Automation
{
    public partial class ImportConfigForm : Form
    {
        ...
        // ── SHARED YELLOW slab thicknesses — defined once on Slab Thickness Rules tab ──
        // Applies to ALL floor types (no per-floor duplication)
        internal NumericUpDown numLobbySlabThickness;
        internal NumericUpDown numStairSlabThickness;
        internal NumericUpDown numFireTenderSlab;
        internal NumericUpDown numOHTSlab;
        internal NumericUpDown numTerraceFireSlab;
        internal NumericUpDown numUGTSlab;
        internal NumericUpDown numLandscapeSlab;
        internal NumericUpDown numSwimmingSlab;
        internal NumericUpDown numDGSlab;
        internal NumericUpDown numSTPSlab;
        ...
        public ImportConfigForm()
        {
            ...
            // Slab thickness dicts
            // Yellow slab thickness controls initialized in InitializeSlabThicknessRulesTab
        ...
        private Dictionary<string, string> GetBeamWallLoadSetsForFloor(string ft)
        {
            // Load pattern name is fixed — always "WALL LOAD" (TextBox removed from UI)
            const string WL = "WALL LOAD";
            return new Dictionary<string, string>
            {
                ["InternalGravity"] = WL,
                ["CantileverGravity"] = WL,
                ["NoLoadGravity"] = "",   // intentionally no load
                ["EdeckGravity"] = WL,
                ["PodiumGravity"] = WL,
                ["GroundGravity"] = WL,
                ["BasementGravity"] = WL,
                ["CoreMain"] = WL,
                ["PeripheralDeadMain"] = WL,
                ["PeripheralPortalMain"] = WL,
                ["InternalMain"] = WL,
            };
        }
        /// <summary>
        ...
        private Dictionary<string, int> GetSlabThicknessesForFloor(string ft)
        {
            // Yellow slab thicknesses are now SHARED — defined once on Slab Thickness Rules tab
            // and applied identically to all floor types.
            int Safe(NumericUpDown nud, int fallback) => nud != null ? (int)nud.Value : fallback;
            return new Dictionary<string, int>
            {
                ["Lobby"] = Safe(numLobbySlabThickness, 160),
                ["Stair"] = Safe(numStairSlabThickness, 175),
                ["FireTender"] = Safe(numFireTenderSlab, 200),
                ["OHT"] = Safe(numOHTSlab, 200),
                ["TerraceFire"] = Safe(numTerraceFireSlab, 200),
                ["UGT"] = Safe(numUGTSlab, 250),
                ["Landscape"] = Safe(numLandscapeSlab, 175),
                ["Swimming"] = Safe(numSwimmingSlab, 250),
                ["DG"] = Safe(numDGSlab, 200),
                ["STP"] = Safe(numSTPSlab, 200),
            };
        }
        ...
        private Dictionary<string, SlabLoads> GetSlabIndividualLoadsForFloor(string ft)
        {
            ...
            {
                var c = kv.Value; // [FF, Fill, ASDL, LL, LL>3, FireTender, Tree, Machine, WaterTank]
                // c[5..8] may be null for regular slabs (only 5 cols) — default to 0
                double V(int i) => (c[i] != null) ? (double)c[i].Value : 0.0;
                result[kv.Key] = new SlabLoads(
                    V(0),  // FF
                    V(1),  // Filling
                    V(2),  // ASDL
                    V(3),  // LL
                    V(4),  // LL3
                    V(5),  // FireTender   (special slabs only)
                    V(6),  // WaterTank    (special slabs only)
                    V(7),  // TreeLoad     (special slabs only)
                    V(8)   // MachineRoom  (special slabs only)
                );
            }
            return result;
        }
