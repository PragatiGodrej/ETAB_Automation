...
namespace ETAB_Automation
{
    public partial class ImportConfigForm
    {
        ...
        // ====================================================================
        // Yellow slab thicknesses moved to Slab Thickness Rules tab (shared, not per-floor)
        ...
        private void InitializeLoadSetsTab(TabPage tab)
        {
            ...
            // AFTER
            const int bLblW = 230, bTxX = 245, bTxW = 210;  // ← increased from 175 to 210
            const int magNudX = 260, magNudW = 90, bRH = 32; // ← shift magNudX right by 35 to match
            int by = 56;
                ...
                out System.Windows.Forms.TextBox txOut, out NumericUpDown nudOut)
            {
                ...
                // Load pattern name hardcoded to "WALL LOAD" — not shown in UI
                txOut = null;
            ...
            var colBg5 = new System.Drawing.Color[]
            {
                System.Drawing.Color.FromArgb(255, 245, 200),
                System.Drawing.Color.FromArgb(255, 235, 180),
                System.Drawing.Color.FromArgb(220, 235, 255),
            ...
            var colBg9 = new System.Drawing.Color[]
            {
                System.Drawing.Color.FromArgb(255, 245, 200),
                System.Drawing.Color.FromArgb(255, 235, 180),
                System.Drawing.Color.FromArgb(220, 235, 255),
            ...
            string[] hdrs5 = { "FF", "FILLING", "ASDL", "LL", "LL>3" };
            string[] hdrs9 = { "FF", "FILLING", "ASDL", "LL", "LL>3", "FIRE", "WATER", "TREE", "MACH" };
            string[] tips5 = {
                "Floor Finish (kN/m²)", "Filling (kN/m²)",
                "ASDL — Additional Superimposed Dead Load (kN/m²)",
                "Live Load ≤ 3m height (kN/m²)", "Live Load > 3m height (kN/m²)"
            };
            string[] tips9 = {
                "Floor Finish (kN/m²)", "Filling (kN/m²)",
                "ASDL — Additional Superimposed Dead Load (kN/m²)",
                "Live Load ≤ 3m height (kN/m²)", "Live Load > 3m height (kN/m²)",
                "Fire Tender Load (kN/m²)", "Water Tank Load (kN/m²)",
                "Tree Load (kN/m²)", "Machine Room Load (kN/m²)"
            };

            // Helper: build a header row inside a group
            void BuildHeaders(GroupBox grp, string[] hdrs, string[] tips)
            {
                for (int ci = 0; ci < hdrs.Length; ci++)
                {
                    int hx = lblW + 8 + ci * (nudW + nudGp);
                    ...
                    toolTip.SetToolTip(lbl, tips[ci]);
                }
            }

            // Helper: build one slab row, fill nuds[0..nCols-1], pad rest with zeros
            void BuildSlabRow(GroupBox grp, string sLbl, string sKey,
                int ri, int nCols, System.Drawing.Color[] bg, string[] colTips)
            {
                int ry = dataY + ri * rowH;
                AddLabel(grp, sLbl + ":", 8, ry + 4, lblW, 20, fontSize: 9f);

                FloorTypeConfig.DefaultSlabIndividualLoads.TryGetValue(sKey, out SlabLoads def);
                def = def ?? new SlabLoads(0, 0, 1, 2);

                double[] allDefaults = {
                    def.FF, def.Filling, def.ASDL,
                    def.LL, def.LL3, def.FireTender,
                    def.WaterTank, def.TreeLoad, def.MachineRoom
                };

                var nuds = new NumericUpDown[9]; // always 9 — unused slots stay null / zero
                for (int ci = 0; ci < nCols; ci++)
                {
                    ...
                    nuds[ci] = nud;
                    toolTip.SetToolTip(nud, $"{sLbl} — {colTips[ci]}");
                }
                // Slots beyond nCols stay null → GetSlabIndividualLoadsForFloor reads 0
                sharedSlabIndividualLoadControls[sKey] = nuds;
            }

            // ── GROUP 1: Regular slabs — 5 columns (FF, FILLING, ASDL, LL, LL>3) ──
            var regularSlabs = new (string lbl, string key)[]
            {
                ("S-Cantilever BALCONY",   "Balcony"),
                ("S-Cantilever CHAJJA",    "Chajja"),
                ("S-Cantilever CHAJJA+ODU","ChajjaODU"),
            ...
            int grp1W = lblW + (nudW + nudGp) * 5 + 30;
            int grp1H = dataY + regularSlabs.Length * rowH + 36;
            var grpSlab = AddGroupBox(tab,
                "Slab Individual Loads — Regular  (FF | FILLING | ASDL | LL | LL>3)  kN/m²",
                20, y, Math.Max(grp1W + 20, 700), grp1H);
            AddLabel(grpSlab,
                "Values assigned to ETABS load patterns. Zero = pattern skipped.",
                15, subtitleY, 660, 16, italic: true, color: System.Drawing.Color.DarkGreen, fontSize: 9f);
            BuildHeaders(grpSlab, hdrs5, tips5);
            for (int ri = 0; ri < regularSlabs.Length; ri++)
                BuildSlabRow(grpSlab, regularSlabs[ri].lbl, regularSlabs[ri].key, ri, 5, colBg5, tips5);

            y += grp1H + 16;

            // ── GROUP 2: Special slabs — 9 columns (FF..LL>3 + FIRE, WATER, TREE, MACH) ──
            var specialSlabs = new (string lbl, string key)[]
            {
                ("S-AMENITIES",          "Amenities"),
                ("S-FIRE TENDER",        "FireTender"),
                ("S-FIRE WATER TANK",    "FireWaterTank"),
            ...
            int grp2W = lblW + (nudW + nudGp) * 9 + 30;
            int grp2H = dataY + specialSlabs.Length * rowH + 36;
            var grpSpecial = AddGroupBox(tab,
                "Slab Individual Loads — Special  (FF | FILLING | ASDL | LL | LL>3 | FIRE | WATER | TREE | MACH)  kN/m²",
                20, y, Math.Max(grp2W + 20, 960), grp2H);
            AddLabel(grpSpecial,
                "These slabs carry special loads (Fire Tender, Water Tank, Tree, Machine Room) in addition to standard loads.",
                15, subtitleY, 900, 16, italic: true, color: System.Drawing.Color.DarkRed, fontSize: 9f);
            BuildHeaders(grpSpecial, hdrs9, tips9);
            for (int ri = 0; ri < specialSlabs.Length; ri++)
                BuildSlabRow(grpSpecial, specialSlabs[ri].lbl, specialSlabs[ri].key, ri, 9, colBg9, tips9);
        ...
        // WHITE slabs  → area-based rules  (m²  → mm thickness)
        // CYAN slabs   → cantilever span rules (m → mm thickness)
        // YELLOW slabs → fixed user thickness (defined once here, shared across all floor types)
        // ====================================================================
        ...
        private void InitializeSlabThicknessRulesTab(TabPage tab)
        {
            ...
            // ── YELLOW slab thicknesses — SHARED for all floors ──────────────
            var grpYellow = AddGroupBox(tab,
                "🟡 YELLOW Slab — Fixed Thickness  (shared across ALL floor types)",
                20, y, 910, 220);
            AddLabel(grpYellow,
                "These thicknesses apply to every floor type. No need to set them per-floor.",
                15, 18, 880, 16, italic: true, color: System.Drawing.Color.DarkGreen, fontSize: 8.5f);

            const int yCol1L = 20, yCol1N = 160;
            const int yCol2L = 470, yCol2N = 610;
            const int yNW = 85, yNH = 25, yRH = 32;
            int yr = 40;

            NumericUpDown AddYellowShared(Control p, string lbl, int lx, int nx, int ry, int defVal)
            {
                AddLabel(p, lbl, lx, ry + 3, nx - lx - 4, 20);
                var n = AddNumericCtrl(p, nx, ry, 100, 600, defVal, increment: 5);
                AddLabel(p, "mm", nx + yNW + 4, ry + 3, 30, 20, color: System.Drawing.Color.DimGray);
                return n;
            }

            numLobbySlabThickness = AddYellowShared(grpYellow, "S-LOBBY:", yCol1L, yCol1N, yr, 160);
            numStairSlabThickness = AddYellowShared(grpYellow, "S-STAIRCASE:", yCol1L, yCol1N, yr += yRH, 175);
            numFireTenderSlab = AddYellowShared(grpYellow, "S-FIRE TENDER:", yCol1L, yCol1N, yr += yRH, 200);
            numOHTSlab = AddYellowShared(grpYellow, "S-OHT:", yCol1L, yCol1N, yr += yRH, 200);
            numTerraceFireSlab = AddYellowShared(grpYellow, "S-TERRACE FIRE:", yCol1L, yCol1N, yr += yRH, 200);

            yr = 40;
            numUGTSlab = AddYellowShared(grpYellow, "S-UGT:", yCol2L, yCol2N, yr, 250);
            numLandscapeSlab = AddYellowShared(grpYellow, "S-LANDSCAPE:", yCol2L, yCol2N, yr += yRH, 175);
            numSwimmingSlab = AddYellowShared(grpYellow, "S-SWIMMING:", yCol2L, yCol2N, yr += yRH, 250);
            numDGSlab = AddYellowShared(grpYellow, "S-DG:", yCol2L, yCol2N, yr += yRH, 200);
            numSTPSlab = AddYellowShared(grpYellow, "S-STP:", yCol2L, yCol2N, yr += yRH, 200);
