...
//        // ── Slab thickness rules (WHITE layers — area-based) ─────────────────
//        // List of (thicknessMm, maxAreaM2) in ascending area order.
//        // If null/empty, SlabImporter uses DefaultSlabAreaRules.
//        public List<(int thickness, double maxArea)> SlabAreaRules { get; set; } = null;

//        // ── Slab thickness rules (CYAN layers — cantilever span-based) ───────
//        // List of (thicknessMm, maxSpanM) in ascending span order.
//        // If null/empty, SlabImporter uses DefaultSlabCantileverRules.
//        public List<(int thickness, double maxSpan)> SlabCantileverRules { get; set; } = null;

//        // ── Static defaults (used when no UI override provided) ──────────────
//        public static readonly List<(int thickness, double maxArea)> DefaultSlabAreaRules
//            = new List<(int, double)>
//            {
...
//        public static readonly List<(int thickness, double maxSpan)> DefaultSlabCantileverRules
//            = new List<(int, double)>
//            {
...
namespace ETAB_Automation.Models
{
    ...
    public class FloorTypeConfig
    {
        ...
        // ── Beam wall load magnitudes (kN/m, user-entered; multiply ×1000 → N/m) ──
        // Key = same as BeamWallLoadSets (e.g. "InternalGravity")
        // Value = load in kN/m as entered by user (BeamImporter multiplies ×1000 internally)
        public Dictionary<string, double> BeamWallLoadMagnitudes { get; set; }
